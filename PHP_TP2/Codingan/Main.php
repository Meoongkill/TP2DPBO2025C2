...
if (!isset($_SESSION['petshop_data']) || !isset($_SESSION['initialized']) || $_SESSION['initialized'] == false) {
    ...
    $item1 = new PetShop();
    $item1->settingDataPetshop(1, "Dog Food Premium", "Makanan", 75000, 50, "Vault_Photo/Atago.jpg");
    $_SESSION['petshop_data'][$_SESSION['item_count']++] = serialize($item1);
    
    
    $item2 = new Aksesories(-1, "", "", 0, 0, "");
    $item2->settingDataAksesories(2, "Dog Collar", "Aksesoris", 45000, 10, "Vault_Photo/Nakano_Miku.jpg", "Collar", "Leather", "Black");
    $_SESSION['petshop_data'][$_SESSION['item_count']++] = serialize($item2);
    
    $item3 = new Aksesories(-1, "", "", 0, 0, "");
    $item3->settingDataAksesories(3, "Cat Toy Mouse", "Aksesoris", 25000, 15, "Vault_Photo/Violet_Evergarden.jpg", "Toy", "Fabric", "Grey");
    $_SESSION['petshop_data'][$_SESSION['item_count']++] = serialize($item3);
    
    
    $item4 = new Baju(-1, "", "", 0, 0, "", "", "", "");
    $item4->settingDataBaju(4, "Dog Sweater", "Baju", 85000, 8, "Vault_Photo/Yae_Miko.jpg", "Sweater", "Cotton", "Red", "Dog", "M", "PetStyle");
    $_SESSION['petshop_data'][$_SESSION['item_count']++] = serialize($item4);
    
    $item5 = new Baju(-1, "", "", 0, 0, "", "", "", "");
    $item5->settingDataBaju(5, "Cat Costume", "Baju", 95000, 12, "Vault_Photo/Lisa.jpg", "Costume", "Polyester", "Blue", "Cat", "S", "Pawfashion");
    ...
    if ($menu == 1) { 
        ...
        if ($_SESSION['item_count'] >= 100) {
            ...
            if ($_POST['object_type'] == 'petshop') {
                ...
                $newItem = new PetShop();
                $newItem->settingDataPetshop($id, $name, $category, $price, $stock, $image_path);
            } 
                ...
                $newItem = new Aksesories(-1, "", "", 0, 0, "");
                $newItem->settingDataAksesories($id, $name, $category, $price, $stock, $image_path, $type, $material, $color);
            }
                ...
                $newItem->settingDataBaju($id, $name, $category, $price, $stock, $image_path, $type, $material, $color, $forwho, $size, $brand);
            }
